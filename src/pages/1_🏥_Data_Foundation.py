...
def display_environment_status():
    """Display environment and AI capabilities status"""
    ...
    with col2:
        ...
        ai_status = test_cortex_ai_functions()
        
        model_names = {
            "mistral-large": "Mistral Large (Clinical Summaries)",
            "mixtral-8x7b": "Mixtral 8x7B (Diagnostics)",
            "llama3.1-70b": "Llama 3.1 70B (Education)",
            "sentiment": "Sentiment Analysis",
            "summarize": "Text Summarization"
        }
        
        for model, name in model_names.items():
            if ai_status.get(model, False):
                st.markdown(f"<span class='status-indicator status-green'></span>✓ {name}", 
                          unsafe_allow_html=True)
            else:
                st.markdown(f"<span class='status-indicator status-yellow'></span>⚠ {name}", 
                          unsafe_allow_html=True)
...
def main():
    ...
    if not processing_data.empty and processing_data.iloc[0].get('processed_count', processing_data.iloc[0].get('PROCESSED_COUNT', 0)) > 0:
        ...
        1. Run `python scripts/process_patients.py` to analyze patient notes
